        ...
        <section class="section">
            ...
            <div class="card" id="output" style="display:none;">
                <div class="card-body table-responsive" style="background-color:white">
                    <div class="card-header" id="crd" style="background-color:white">
                        <br class="mb-4">
                    ...
                    <div style="background-color:white" class="card-body mb-3" id="areaChart"></div>
...
@section('scripts')
    <script>
        ...
        $('#menu-generateReport-waterLevel').addClass('active')
        ...
        function GeneratePDF(){
            ...
            // };
            // html2pdf().set(opt).from(output).save('FLSys-GenerateReport-'+today+'.pdf');
        }
        ...
        $(document).ready( 
            function () {  
                $('#generate').on('click', function () {
                    ...
                    // AJAX
                    $.ajax({
                    url: '{{ route('generateWaterLevel') }}?from='+generateFrom+'&to='+generateTo+'&device='+deviceId,
                    type: 'get',
                    success: function ([data,table]) {
                        if(data == 0 && table ==0){
                            ...
                            $('#crd_a').html('Date : '+dp);
                            $('#crd_b').html(data.data.height.length);
                            $('#crd_c').html($('#generate_from').val()+' - '+$('#generate_to').val());
                            $('#crd_d').html(data.data.device_name.toUpperCase());
                            var devname = data.data.device_name.toUpperCase();
                            $('#tb1').html(
                                '\n Date : '+dp+
                                '\n\nTotal Data : '+data.length+
                                '\n\n Date to be Export : '+$('#generate_from').val()+' - '+$('#generate_to').val()+
                                '\n\n Device/s Name : '+devname+
                                '\n\n Color Level : GREEN, ORANGE, RED'+
                                '\n\n'
                            );
                            // Chart
                            chart.render();
